...
The solution is much more straightforward and simple if we want to solve this dynamically - simply place `key.bin` in `C:\CTF`, place a BP where before the `cmp` comparing the result from the call to CheckTokenMembership, and flip the logic from `jne` to a `je` - the program will return the decrypted flag once you continue from your BP 😄 
